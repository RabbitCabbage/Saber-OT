...
class NPSaber2: public OT<IO> {
    ...
    void send(const block* data0, const block* data1,int64_t length) override {
        ...
        for (int64_t i = 0; i < length; ++i) {
            ...
            unsigned char pack_info[SABER_POLYVECCOMPRESSEDBYTES + SABER_SCALEBYTES_KEM * 2 + 2 * sizeof(block)];
            ...
            POLT2BS(pack_info + SABER_POLYVECCOMPRESSEDBYTES, cm0);
            POLT2BS(pack_info + SABER_POLYVECCOMPRESSEDBYTES + SABER_SCALEBYTES_KEM, cm1);
            ...
            memcpy(pack_info + SABER_POLYVECCOMPRESSEDBYTES + 2 * SABER_SCALEBYTES_KEM, m, 2 * sizeof(block));
            io->send_data(pack_info, SABER_POLYVECCOMPRESSEDBYTES + 2 * SABER_SCALEBYTES_KEM + 2 * sizeof(block));
        }
    ...
    void recv(block* data, const bool* x, int64_t length) override {
        ...
        for (int64_t i = 0; i < length; ++i) {
            ...
            unsigned char pack_info[SABER_POLYVECCOMPRESSEDBYTES + SABER_SCALEBYTES_KEM * 2 + 2 * sizeof(block)];
            io->recv_data(pack_info, SABER_POLYVECCOMPRESSEDBYTES + SABER_SCALEBYTES_KEM * 2 + 2 * sizeof(block));
            // BS2POLVECp(compressed_bp, b0[i]);
            BS2POLVECp(pack_info, b0[i]);
            ...
            BS2POLT(pack_info + SABER_POLYVECCOMPRESSEDBYTES, cm0);
            // uint8_t compressed_cm1 [SABER_SCALEBYTES_KEM];
            // io->recv_data(compressed_cm1, SABER_SCALEBYTES_KEM * sizeof(uint8_t));
            // BS2POLT(compressed_cm1, cm1);
            BS2POLT(pack_info + SABER_POLYVECCOMPRESSEDBYTES + SABER_SCALEBYTES_KEM, cm1);
            // io->recv_data(m, 2 * sizeof(block));
            memcpy(m, pack_info + SABER_POLYVECCOMPRESSEDBYTES + 2 * SABER_SCALEBYTES_KEM, 2 * sizeof(block));
